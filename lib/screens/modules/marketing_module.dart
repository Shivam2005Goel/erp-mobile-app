...
/// Marketing: Meta Ads, Google Ads spend analytics, and content calendar.
...
class _GoogleAds extends StatelessWidget {
  final ErpRepository repo;
  const _GoogleAds({required this.repo});
  ...
  Widget build(BuildContext context) {
    return AsyncSection<List<Map<String, dynamic>>>(
      loader: () async {
        final all = await repo.adsCosting();
        return all
            .where((r) =>
                (r['platform']?.toString().toLowerCase() ?? '') == 'google')
            .toList();
      },
      isEmpty: (d) => d.isEmpty,
      emptyMessage: 'No Google Ads data found.',
      builder: (context, rows, refresh) {
        final spend = rows.fold<num>(0, (s, r) => s + _n(r['spend']));
        final clicks = rows.fold<num>(0, (s, r) => s + _n(r['clicks']));
        final impressions =
            rows.fold<num>(0, (s, r) => s + _n(r['impressions']));
        final campaigns =
            rows.map((r) => r['campaign_id'] ?? r['campaign_name']).toSet();
          ...
          children: [
            ...
            ...rows.take(40).map((r) => Card(
                  ...
                  child: ListTile(
                    ...
                    title: Text(
                        str(r['campaign_name'] ?? r['ad_name'],
                            str(r['campaign_id']))),
                    subtitle: Text(
                        '${fmtDate(r['date'])} • ${str(r['objective'] ?? r['ad_type'], '—')}'),
                    trailing: Column(
                      mainAxisAlignment: MainAxisAlignment.center,
                      crossAxisAlignment: CrossAxisAlignment.end,
                      children: [
